...
class WorldViewer(basis.Entity):
    ...
    def draw_info_window(self):
        ...
        if not self.agent:
            if self.agent_id:
                ...
                self.agent = basis.first_of(self.system.get_entities_by_type_recursively(core.Agent))
    ...
    def draw_board(self, pos, size):
        ...
        for obstacle in self.board.obstacles:
            if isinstance(obstacle, core.Obstacle):
                try:
        ...
        for agent in self.board.agents:
            if isinstance(agent, core.Agent):
                try:
    ...
    def draw(self):
        ...
        glfw.poll_events()
        glfw.set_window_title(self.window, "Lambert")
